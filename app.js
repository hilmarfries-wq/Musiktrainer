...
const MUSIC_ACCIDENTAL_BOUNDS={"sharp": [51, -233, 502, 590], "flat": [51, -25, 296, 524], "natural": [51, -213, 306, 404]};
...
function accidentalSvg(kind,x,y){
 ...
 const b=MUSIC_ACCIDENTAL_BOUNDS[kind];
 if(!path||!b)return "";

 const scale=kind==="flat"?0.052:kind==="sharp"?0.050:0.048;
 const glyphCenterX=(b[0]+b[2])/2;
 const glyphCenterY=(b[1]+b[3])/2;

 const desiredCenterX=x-48;
 const desiredCenterY=y;

 const tx=desiredCenterX-glyphCenterX*scale;
 const ty=desiredCenterY+glyphCenterY*scale;

 return `<path d="${path}" transform="translate(${tx} ${ty}) scale(${scale} -${scale})" fill="#182033"/>`;
}
